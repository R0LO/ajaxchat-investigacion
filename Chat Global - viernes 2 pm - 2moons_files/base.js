function NewTabs(container, btnContainer, id, t)
{
	if (!$(container).find(id).hasClass('inactive-tab')) {return;}

	$(container).find('.active-tab').addClass('inactive-tab').hide();
	$(container).find(id).addClass('active-tab').fadeIn(500).removeClass('inactive-tab');
	$(btnContainer).find('.btn-success').removeClass('btn-success').addClass('btn-primary');
...
function DestroyMissiles() {
	$.getJSON('?page=information&mode=destroyMissiles&'+$('.missile').serialize(), function(data) {
		...
		$('#missile_504').text(NumberGetHumanReadable(data[2]));
		$('.missile').val('');
	});
}
...
var Dialog	= {	
	info: function(ID){
		return Dialog.open('game.php?page=information&id='+ID, 590);
	},
	...
	PM: function(ID, Subject, Message) {
		...
		return Dialog.open('game.php?page=messages&mode=write&id='+ID+'&subject='+encodeURIComponent(Subject)+'&message='+encodeURIComponent(Subject), 650);
	},
	
	Playercard: function(ID) {
		return isPlayerCardActive && Dialog.open('game.php?page=playerCard&id='+ID, 650);
	},

	Obg: function(ID) {
		return Dialog.open('game.php?page=Obg&mode=obgAdd&id='+ID, 550);
	},
	
	Buddy: function(ID) {
		return Dialog.open('game.php?page=buddyList&mode=request&id='+ID, 650);
	},
	
	SRTF: function(ID) {
		return Dialog.open('game.php?page=messages&mode=SRTFshow&RaportID='+ID, 400);
	},

	ACS: function(ID) {
		return Dialog.open('game.php?page=Flight&mode=ACS&fleetID='+ID, 400);
	},

	MIP: function(x, y, p, t) {
		return Dialog.open('game.php?page=galaxy&mode=ShowMisiles&x='+x+'&y='+y+'&p='+p+'&t='+t, 650);
	},

	NUC: function(x, y, p, t) {
		return Dialog.open('game.php?page=galaxy&mode=ShowNuclear&x='+x+'&y='+y+'&p='+p+'&t='+t, 650);
	},
...
$.fn.loading = function (state) {
  ...
  if (state === true) {
    t.html('<div id="BackGroundFreezerPreloader_Element" class="BackGroundFreezer_Element" align="center"><div class="loader"></div></div>');
  } else {
    $('#BackGroundFreezerPreloader_Element').remove();
  }
...
function copi(id) {
  	...
  	parent.alerta('success', 'El link de referencia ha sido copiado en su portapapeles', 'System');
}
...
$(function() {
	...
	$('#MplanetSelector, #planetSelector').on('change', function() {
