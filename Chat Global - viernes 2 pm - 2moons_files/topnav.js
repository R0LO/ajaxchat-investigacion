...
function resourceTicker(config, init) {
	...
	if (nrResource < config.limit[1]) 
	{
		...
		if(viewShortlyNumber) {
			...
			element.html(shortly_number(nrResource) + " <span class='d-n'>["+pourcent+"%]</span> ");
			elementPoursent.css('width', pourcent+'%');

		} else {
			element.html(NumberGetHumanReadable(nrResource) + " <span class='d-n'>["+pourcent+"%]</span> ");
			elementPoursent.css('width', pourcent+'%');
		...
		element.html(shortly_number(nrResource) + " <span class='d-n'>[100%]</span> ");
	}
}
